...
Machine-readable JSON now uses standard ASCII escaping, preserving the same parsed Unicode values while remaining writable through legacy Windows/charmap stdout used by hosted runners and redirected consoles.
...
- An ASCII-safety regression parses the escaped desktop-smoke JSON back to the original contract.
